...
namespace TeamYaffa.CRaPI.Info
{
	...
	/// </summary>
	/// <remarks>This class is used for the player characteristics sent by the server
	/// when the player connects.</remarks>
	public class PlayerType : InfoBase
	{
		...
		/// <summary>Parses the <c>player_type</c> message from the server</summary>
		/// <remarks><see cref="PlayerType"/> is able to parse <c>player_type</c> messages from robocup server 
		/// 9.0.4. Changes to the protocol could require changes to this class.</remarks>
		/// <param name="pServerData">The message from the server. Must begin with <c>(player_type</c>.</param>
		/// <returns>A ServerParam object if it was a valid <c>player_type</c> message. Null otherwise.</returns>
		public static PlayerType Create(string pServerData)
		{
			if(!pServerData.StartsWith("(player_type"))
				return null;

			PlayerType result = new PlayerType();
